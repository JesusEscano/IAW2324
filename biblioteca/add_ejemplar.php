...
// Verificar si se recibió el ID del ejemplar a agregar
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id_ejemplar_agregar'])) {
    $id_ejemplar = intval($_GET['id_ejemplar_agregar']);

    // Insertar otro ejemplar del mismo libro en la base de datos con la misma estantería
    $sql_insert_ejemplar = "INSERT INTO ejemplares (id_libro, estanteria, estado) SELECT id_libro, estanteria, estado FROM ejemplares WHERE id_ejemplar = ?";
    ...
    // Si no se recibió el ID del ejemplar a agregar, redirigir a la página principal
    header("Location: verejemplares.php");
    exit(); 
}
